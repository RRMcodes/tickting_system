...
@section('content')
    ...
    <script>
        $(document).ready( function () {
            $('#myTable').DataTable({
                pageLength: 8,
                ajax : '{{route('eventTypes.getEventTypesJson')}}',
                columns: [
                    ...
                    { data: function(data,b,c,table) {
                            var buttons = '';
                            buttons += "<a class='btn btn-sm btn-primary'  href='{{route('eventTypes.edit',array('id'=>false))}}"+"/"+data.id+"' type='button' title='Edit'><i class='fa fa-edit'></i></a>&nbsp";
        ...
        $(document).on('click','.del',function (){
            var id = $(this).data('id');
            alertify.confirm('Delete?', 'Are You Sure you want to delete this record?', function(){	$.get("{{route('eventTypes.delete',array('id'=>false))}}"+"/"+id,function (d) {
